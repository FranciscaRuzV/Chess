...
function initGame(){
    ...
    const config = {
        ...
        onDrop: onDrop,
        onSnapEnd: onSnapEnd,
        pieceTheme: '/public/img/pieces-basic-png/{piece}.png'
    };
...
socket.on('bothPlayersReady',function(){
    console.log('Both players ready, game strarting');
    gameHasStarted = true;
...
socket.on('move',function(moveData){
    ...
    if(moveData.color !== playerColor){
        const move = {
            from: moveData.from,
            to: moveData.to,
            promotion: moveData.promotion || 'q' //default to queen if it's not specific
        };
        game.move(move);
...
/**
 * Handle piece drop on thee board
 */
...
/**
 * Update board position after the piece snp animation
 */
function onSnapEnd(){
    board.position(game.fen());
}
